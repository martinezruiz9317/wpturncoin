<?php
get_header();
?>
<div id="fullpage">
    <div id="homeHeroStc" class="section">
        <?php get_template_part('templates/home', 'people'); ?>
    </div>
    <div id="blockSection" class="section fp-auto-height">
        <div class="container text-center">
            <h2 class="c-gold">World-First People’s Exchange</h2>
            <p>People are passionate beings, made to create and achieve greatness. </p>
            <p style="max-width: 820px; margin: 0 auto 10px" class="x-light">Our platform will have the ability to generate specialized sub-exchanges worldwide, in any language and in any currency. It will follow the same unique and insanely-simple business model that can be duplicated regionally internationally, nationally, and even for event-specific exchanges.</p>
            <div class="my-row home-block-stc">
                <div class="my-col-4">
                    <div class="block-inner d-flex flex-column align-items-center">
                        <div class="block-icon d-flex align-items-center justify-content-center">
                            <svg xmlns="http://www.w3.org/2000/svg" width="61.574" height="66.463" viewBox="0 0 61.574 66.463">
                                <g id="Group_839" data-name="Group 839" transform="translate(-522.875 -1907.254)">
                                    <g id="Group_478" data-name="Group 478" transform="translate(536.286 1920.357)">
                                        <g id="Group_477" data-name="Group 477">
                                            <path id="Path_547" data-name="Path 547" d="M104.5,85.12A17.388,17.388,0,0,0,87.12,102.5a19.778,19.778,0,0,0,8.214,15.775V130.82a1.262,1.262,0,0,0,.086.419v.1a1.319,1.319,0,0,0,.074.148l4.335,6.453a1.235,1.235,0,0,0,1.022.542h7.167a1.235,1.235,0,0,0,1.022-.542l4.335-6.453a1.32,1.32,0,0,0,.074-.148v-.1a1.263,1.263,0,0,0,.086-.419V118.271a19.778,19.778,0,0,0,8.214-15.775A17.388,17.388,0,0,0,104.5,85.12Zm2.931,50.934h-5.862l-2.685-3.99H110.1ZM97.809,129.6v-3.263h13.374l.012,3.263Zm13.94-13.017v.037a1.233,1.233,0,0,0-.554,1.034v6.256H97.809v-6.33a1.234,1.234,0,0,0-.554-1.034,17.243,17.243,0,0,1-7.66-14.088,14.913,14.913,0,0,1,29.814,0A17.239,17.239,0,0,1,111.75,116.584Z" transform="translate(-87.12 -85.12)" fill="#d0ad6a" />
                                        </g>
                                    </g>
                                    <g id="Group_480" data-name="Group 480" transform="translate(552.923 1925.935)">
                                        <g id="Group_479" data-name="Group 479">
                                            <path id="Path_548" data-name="Path 548" d="M196.431,121.36a1.231,1.231,0,0,0,0,2.463A9.285,9.285,0,0,1,205.7,133.1a1.231,1.231,0,1,0,2.463,0A11.747,11.747,0,0,0,196.431,121.36Z" transform="translate(-195.2 -121.36)" fill="#d0ad6a" />
                                        </g>
                                    </g>
                                    <g id="Group_482" data-name="Group 482" transform="translate(522.875 1936.44)">
                                        <g id="Group_481" data-name="Group 481">
                                            <path id="Path_549" data-name="Path 549" d="M10.1,189.6H1.231a1.231,1.231,0,0,0,0,2.463H10.1a1.231,1.231,0,0,0,1.231-1.231h0A1.231,1.231,0,0,0,10.1,189.6Z" transform="translate(0 -189.6)" fill="#d0ad6a" />
                                        </g>
                                    </g>
                                    <g id="Group_484" data-name="Group 484" transform="translate(573.119 1936.44)">
                                        <g id="Group_483" data-name="Group 483">
                                            <path id="Path_550" data-name="Path 550" d="M336.5,189.6h-8.867a1.231,1.231,0,0,0,0,2.463H336.5a1.231,1.231,0,1,0,0-2.463Z" transform="translate(-326.4 -189.6)" fill="#d0ad6a" />
                                        </g>
                                    </g>
                                    <g id="Group_486" data-name="Group 486" transform="translate(531.434 1951.021)">
                                        <g id="Group_485" data-name="Group 485">
                                            <path id="Path_551" data-name="Path 551" d="M64.2,284.752a1.231,1.231,0,0,0-1.87,0v-.025l-6.293,6.317a1.231,1.231,0,1,0,1.736,1.736l6.293-6.293A1.23,1.23,0,0,0,64.2,284.752Z" transform="translate(-55.601 -284.321)" fill="#d0ad6a" />
                                        </g>
                                    </g>
                                    <g id="Group_488" data-name="Group 488" transform="translate(567.065 1915.579)">
                                        <g id="Group_487" data-name="Group 487">
                                            <path id="Path_552" data-name="Path 552" d="M295.331,54.376a1.234,1.234,0,0,0-1.6,0l-6.293,6.293a1.231,1.231,0,0,0,.874,2.106l-.012-.012a1.23,1.23,0,0,0,.874-.357l6.293-6.293A1.233,1.233,0,0,0,295.331,54.376Z" transform="translate(-287.071 -54.08)" fill="#d0ad6a" />
                                        </g>
                                    </g>
                                    <g id="Group_490" data-name="Group 490" transform="translate(566.999 1950.996)">
                                        <g id="Group_489" data-name="Group 489">
                                            <path id="Path_553" data-name="Path 553" d="M295.346,291.157a1.205,1.205,0,0,0-.245-.248l-6.293-6.318a1.231,1.231,0,1,0-1.736,1.736l6.293,6.293a1.232,1.232,0,0,0,1.981-1.464Z" transform="translate(-286.641 -284.161)" fill="#d0ad6a" />
                                        </g>
                                    </g>
                                    <g id="Group_492" data-name="Group 492" transform="translate(531.568 1915.578)">
                                        <g id="Group_491" data-name="Group 491">
                                            <path id="Path_554" data-name="Path 554" d="M64.8,60.667l-6.293-6.293a1.231,1.231,0,0,0-1.736,1.736L63.062,62.4A1.231,1.231,0,0,0,64.8,60.667Z" transform="translate(-56.472 -54.079)" fill="#d0ad6a" />
                                        </g>
                                    </g>
                                    <g id="Group_494" data-name="Group 494" transform="translate(552.43 1907.254)">
                                        <g id="Group_493" data-name="Group 493">
                                            <path id="Path_555" data-name="Path 555" d="M193.228,0A1.231,1.231,0,0,0,192,1.231v8.9a1.233,1.233,0,0,0,1.195,1.268h.037a1.231,1.231,0,0,0,1.231-1.231V1.231A1.231,1.231,0,0,0,193.228,0Z" transform="translate(-191.996)" fill="#d0ad6a" />
                                        </g>
                                    </g>
                                </g>
                            </svg>
                        </div>
                        <h3 class="l-size c-gold">
                            THE MODEL
                        </h3>
                        <p class="x-light">A people-driven platform to help individuals achieve their dreams,
                            while giving their fans a way to support them and share in
                            their success. </p>
                    </div>
                </div>
                <div class="my-col-4">
                    <div class="block-inner d-flex flex-column align-items-center">
                        <div class="block-icon d-flex align-items-center justify-content-center">
                            <svg xmlns="http://www.w3.org/2000/svg" width="61.574" height="66.463" viewBox="0 0 61.574 66.463">
                                <g id="Group_839" data-name="Group 839" transform="translate(-522.875 -1907.254)">
                                    <g id="Group_478" data-name="Group 478" transform="translate(536.286 1920.357)">
                                        <g id="Group_477" data-name="Group 477">
                                            <path id="Path_547" data-name="Path 547" d="M104.5,85.12A17.388,17.388,0,0,0,87.12,102.5a19.778,19.778,0,0,0,8.214,15.775V130.82a1.262,1.262,0,0,0,.086.419v.1a1.319,1.319,0,0,0,.074.148l4.335,6.453a1.235,1.235,0,0,0,1.022.542h7.167a1.235,1.235,0,0,0,1.022-.542l4.335-6.453a1.32,1.32,0,0,0,.074-.148v-.1a1.263,1.263,0,0,0,.086-.419V118.271a19.778,19.778,0,0,0,8.214-15.775A17.388,17.388,0,0,0,104.5,85.12Zm2.931,50.934h-5.862l-2.685-3.99H110.1ZM97.809,129.6v-3.263h13.374l.012,3.263Zm13.94-13.017v.037a1.233,1.233,0,0,0-.554,1.034v6.256H97.809v-6.33a1.234,1.234,0,0,0-.554-1.034,17.243,17.243,0,0,1-7.66-14.088,14.913,14.913,0,0,1,29.814,0A17.239,17.239,0,0,1,111.75,116.584Z" transform="translate(-87.12 -85.12)" fill="#d0ad6a" />
                                        </g>
                                    </g>
                                    <g id="Group_480" data-name="Group 480" transform="translate(552.923 1925.935)">
                                        <g id="Group_479" data-name="Group 479">
                                            <path id="Path_548" data-name="Path 548" d="M196.431,121.36a1.231,1.231,0,0,0,0,2.463A9.285,9.285,0,0,1,205.7,133.1a1.231,1.231,0,1,0,2.463,0A11.747,11.747,0,0,0,196.431,121.36Z" transform="translate(-195.2 -121.36)" fill="#d0ad6a" />
                                        </g>
                                    </g>
                                    <g id="Group_482" data-name="Group 482" transform="translate(522.875 1936.44)">
                                        <g id="Group_481" data-name="Group 481">
                                            <path id="Path_549" data-name="Path 549" d="M10.1,189.6H1.231a1.231,1.231,0,0,0,0,2.463H10.1a1.231,1.231,0,0,0,1.231-1.231h0A1.231,1.231,0,0,0,10.1,189.6Z" transform="translate(0 -189.6)" fill="#d0ad6a" />
                                        </g>
                                    </g>
                                    <g id="Group_484" data-name="Group 484" transform="translate(573.119 1936.44)">
                                        <g id="Group_483" data-name="Group 483">
                                            <path id="Path_550" data-name="Path 550" d="M336.5,189.6h-8.867a1.231,1.231,0,0,0,0,2.463H336.5a1.231,1.231,0,1,0,0-2.463Z" transform="translate(-326.4 -189.6)" fill="#d0ad6a" />
                                        </g>
                                    </g>
                                    <g id="Group_486" data-name="Group 486" transform="translate(531.434 1951.021)">
                                        <g id="Group_485" data-name="Group 485">
                                            <path id="Path_551" data-name="Path 551" d="M64.2,284.752a1.231,1.231,0,0,0-1.87,0v-.025l-6.293,6.317a1.231,1.231,0,1,0,1.736,1.736l6.293-6.293A1.23,1.23,0,0,0,64.2,284.752Z" transform="translate(-55.601 -284.321)" fill="#d0ad6a" />
                                        </g>
                                    </g>
                                    <g id="Group_488" data-name="Group 488" transform="translate(567.065 1915.579)">
                                        <g id="Group_487" data-name="Group 487">
                                            <path id="Path_552" data-name="Path 552" d="M295.331,54.376a1.234,1.234,0,0,0-1.6,0l-6.293,6.293a1.231,1.231,0,0,0,.874,2.106l-.012-.012a1.23,1.23,0,0,0,.874-.357l6.293-6.293A1.233,1.233,0,0,0,295.331,54.376Z" transform="translate(-287.071 -54.08)" fill="#d0ad6a" />
                                        </g>
                                    </g>
                                    <g id="Group_490" data-name="Group 490" transform="translate(566.999 1950.996)">
                                        <g id="Group_489" data-name="Group 489">
                                            <path id="Path_553" data-name="Path 553" d="M295.346,291.157a1.205,1.205,0,0,0-.245-.248l-6.293-6.318a1.231,1.231,0,1,0-1.736,1.736l6.293,6.293a1.232,1.232,0,0,0,1.981-1.464Z" transform="translate(-286.641 -284.161)" fill="#d0ad6a" />
                                        </g>
                                    </g>
                                    <g id="Group_492" data-name="Group 492" transform="translate(531.568 1915.578)">
                                        <g id="Group_491" data-name="Group 491">
                                            <path id="Path_554" data-name="Path 554" d="M64.8,60.667l-6.293-6.293a1.231,1.231,0,0,0-1.736,1.736L63.062,62.4A1.231,1.231,0,0,0,64.8,60.667Z" transform="translate(-56.472 -54.079)" fill="#d0ad6a" />
                                        </g>
                                    </g>
                                    <g id="Group_494" data-name="Group 494" transform="translate(552.43 1907.254)">
                                        <g id="Group_493" data-name="Group 493">
                                            <path id="Path_555" data-name="Path 555" d="M193.228,0A1.231,1.231,0,0,0,192,1.231v8.9a1.233,1.233,0,0,0,1.195,1.268h.037a1.231,1.231,0,0,0,1.231-1.231V1.231A1.231,1.231,0,0,0,193.228,0Z" transform="translate(-191.996)" fill="#d0ad6a" />
                                        </g>
                                    </g>
                                </g>
                            </svg>
                        </div>
                        <h3 class="l-size c-gold">
                            DUAL NATURE
                        </h3>
                        <p class="x-light">TurnCoin™ is a Mega-Security Token that should deliver a perpetual increasing monthly yield and therefore continue to appreciate in value. </p>
                    </div>
                </div>
                <div class="my-col-4">
                    <div class="block-inner d-flex flex-column align-items-center">
                        <div class="block-icon d-flex align-items-center justify-content-center">
                            <svg xmlns="http://www.w3.org/2000/svg" width="61.574" height="66.463" viewBox="0 0 61.574 66.463">
                                <g id="Group_839" data-name="Group 839" transform="translate(-522.875 -1907.254)">
                                    <g id="Group_478" data-name="Group 478" transform="translate(536.286 1920.357)">
                                        <g id="Group_477" data-name="Group 477">
                                            <path id="Path_547" data-name="Path 547" d="M104.5,85.12A17.388,17.388,0,0,0,87.12,102.5a19.778,19.778,0,0,0,8.214,15.775V130.82a1.262,1.262,0,0,0,.086.419v.1a1.319,1.319,0,0,0,.074.148l4.335,6.453a1.235,1.235,0,0,0,1.022.542h7.167a1.235,1.235,0,0,0,1.022-.542l4.335-6.453a1.32,1.32,0,0,0,.074-.148v-.1a1.263,1.263,0,0,0,.086-.419V118.271a19.778,19.778,0,0,0,8.214-15.775A17.388,17.388,0,0,0,104.5,85.12Zm2.931,50.934h-5.862l-2.685-3.99H110.1ZM97.809,129.6v-3.263h13.374l.012,3.263Zm13.94-13.017v.037a1.233,1.233,0,0,0-.554,1.034v6.256H97.809v-6.33a1.234,1.234,0,0,0-.554-1.034,17.243,17.243,0,0,1-7.66-14.088,14.913,14.913,0,0,1,29.814,0A17.239,17.239,0,0,1,111.75,116.584Z" transform="translate(-87.12 -85.12)" fill="#d0ad6a" />
                                        </g>
                                    </g>
                                    <g id="Group_480" data-name="Group 480" transform="translate(552.923 1925.935)">
                                        <g id="Group_479" data-name="Group 479">
                                            <path id="Path_548" data-name="Path 548" d="M196.431,121.36a1.231,1.231,0,0,0,0,2.463A9.285,9.285,0,0,1,205.7,133.1a1.231,1.231,0,1,0,2.463,0A11.747,11.747,0,0,0,196.431,121.36Z" transform="translate(-195.2 -121.36)" fill="#d0ad6a" />
                                        </g>
                                    </g>
                                    <g id="Group_482" data-name="Group 482" transform="translate(522.875 1936.44)">
                                        <g id="Group_481" data-name="Group 481">
                                            <path id="Path_549" data-name="Path 549" d="M10.1,189.6H1.231a1.231,1.231,0,0,0,0,2.463H10.1a1.231,1.231,0,0,0,1.231-1.231h0A1.231,1.231,0,0,0,10.1,189.6Z" transform="translate(0 -189.6)" fill="#d0ad6a" />
                                        </g>
                                    </g>
                                    <g id="Group_484" data-name="Group 484" transform="translate(573.119 1936.44)">
                                        <g id="Group_483" data-name="Group 483">
                                            <path id="Path_550" data-name="Path 550" d="M336.5,189.6h-8.867a1.231,1.231,0,0,0,0,2.463H336.5a1.231,1.231,0,1,0,0-2.463Z" transform="translate(-326.4 -189.6)" fill="#d0ad6a" />
                                        </g>
                                    </g>
                                    <g id="Group_486" data-name="Group 486" transform="translate(531.434 1951.021)">
                                        <g id="Group_485" data-name="Group 485">
                                            <path id="Path_551" data-name="Path 551" d="M64.2,284.752a1.231,1.231,0,0,0-1.87,0v-.025l-6.293,6.317a1.231,1.231,0,1,0,1.736,1.736l6.293-6.293A1.23,1.23,0,0,0,64.2,284.752Z" transform="translate(-55.601 -284.321)" fill="#d0ad6a" />
                                        </g>
                                    </g>
                                    <g id="Group_488" data-name="Group 488" transform="translate(567.065 1915.579)">
                                        <g id="Group_487" data-name="Group 487">
                                            <path id="Path_552" data-name="Path 552" d="M295.331,54.376a1.234,1.234,0,0,0-1.6,0l-6.293,6.293a1.231,1.231,0,0,0,.874,2.106l-.012-.012a1.23,1.23,0,0,0,.874-.357l6.293-6.293A1.233,1.233,0,0,0,295.331,54.376Z" transform="translate(-287.071 -54.08)" fill="#d0ad6a" />
                                        </g>
                                    </g>
                                    <g id="Group_490" data-name="Group 490" transform="translate(566.999 1950.996)">
                                        <g id="Group_489" data-name="Group 489">
                                            <path id="Path_553" data-name="Path 553" d="M295.346,291.157a1.205,1.205,0,0,0-.245-.248l-6.293-6.318a1.231,1.231,0,1,0-1.736,1.736l6.293,6.293a1.232,1.232,0,0,0,1.981-1.464Z" transform="translate(-286.641 -284.161)" fill="#d0ad6a" />
                                        </g>
                                    </g>
                                    <g id="Group_492" data-name="Group 492" transform="translate(531.568 1915.578)">
                                        <g id="Group_491" data-name="Group 491">
                                            <path id="Path_554" data-name="Path 554" d="M64.8,60.667l-6.293-6.293a1.231,1.231,0,0,0-1.736,1.736L63.062,62.4A1.231,1.231,0,0,0,64.8,60.667Z" transform="translate(-56.472 -54.079)" fill="#d0ad6a" />
                                        </g>
                                    </g>
                                    <g id="Group_494" data-name="Group 494" transform="translate(552.43 1907.254)">
                                        <g id="Group_493" data-name="Group 493">
                                            <path id="Path_555" data-name="Path 555" d="M193.228,0A1.231,1.231,0,0,0,192,1.231v8.9a1.233,1.233,0,0,0,1.195,1.268h.037a1.231,1.231,0,0,0,1.231-1.231V1.231A1.231,1.231,0,0,0,193.228,0Z" transform="translate(-191.996)" fill="#d0ad6a" />
                                        </g>
                                    </g>
                                </g>
                            </svg>
                        </div>
                        <h3 class="l-size c-gold">
                            THE REVOLUTION
                        </h3>
                        <p class="x-light">Empower people worldwide to pursue their dreams and passions by listing themselves on a global people’s exchange.</p>
                    </div>
                </div>
            </div>
        </div>
        <div>
            <div class="text-center timel-title-stc">timeline</div>
            <div class="timeline-warp">
                <div class="timeline-top">
                    <div class="timeline-line">
                    </div>
                    <div id="timelineTop">
                        <div class="time-line-block">
                            <div class="timel-top-col d-flex align-items-center">
                                <div class="timel-year">2017</div>
                                <div>商业模型设计</div>
                            </div>
                            <div class="d-flex">
                                <div class="timel-bot-col">
                                    <div style="font-weight: 700; font-size: 20px; margin-bottom: 10px">6月 2017</div>
                                    <div class="c-gold">THE<span style="font-weight: 700;">XCHANGE</span></div>
                                    <div style="font-size: 14px;">项目开启</div>
                                </div>
                                <div class="timel-bot-col">
                                    <div style="font-weight: 700; font-size: 20px; margin-bottom: 10px">&nbsp;</div>
                                    <div class="c-gold">开发启动</div>
                                    <div style="font-size: 14px;">Stellenbosch, 南非</div>
                                </div>
                            </div>
                        </div>
                        <div class="time-line-block">
                            <div class="timel-top-col d-flex align-items-center">
                                <div class="timel-year">2018</div>
                                <div>团队组建&原型开发</div>
                            </div>
                            <div class="d-flex">
                                <div class="timel-bot-col">
                                    <div style="font-weight: 700; font-size: 20px; margin-bottom: 10px">4月 2018</div>
                                    <div class="c-gold">开发推广</div>
                                    <div style="font-size: 14px;">Pretoria Game Lodge<br>南非</div>
                                </div>
                                <div class="timel-bot-col">
                                    <div style="font-weight: 700; font-size: 20px; margin-bottom: 10px">4月 2019</div>
                                    <div class="c-gold">NO REST 实验室</div>
                                    <div style="font-size: 14px;">开启应用程序Beta版本<br>开发</div>
                                </div>
                            </div>
                        </div>
                        <div class="time-line-block">
                            <div class="timel-top-col d-flex align-items-center">
                                <div class="timel-year">2019</div>
                                <div>Beta版开发: TheXchange, SportXchange 应用</div>
                            </div>
                            <div class="d-flex">
                                <div class="timel-bot-col">
                                    <div style="font-weight: 700; font-size: 20px; margin-bottom: 10px">4月29日 2019</div>
                                    <div class="c-gold">TURNCOIN.IO</div>
                                    <div style="font-size: 14px;">平台启动</div>
                                </div>
                                <div class="timel-bot-col">
                                    <div style="font-weight: 700; font-size: 20px; margin-bottom: 10px">8月17日 2019</div>
                                    <div class="c-gold">SX APP-ALPHA STATIC </div>
                                    <div style="font-size: 14px;">苹果 & 安卓</div>
                                </div>
                                <div class="timel-bot-col">
                                    <div style="font-weight: 700; font-size: 20px; margin-bottom: 10px">11月-12月 2019</div>
                                    <div class="c-gold">证券型代币发售</div>
                                    <div style="font-size: 14px;">德州 & 迪拜<br>私募</div>
                                </div>
                            </div>
                        </div>
                        <div class="time-line-block">
                            <div class="timel-top-col d-flex align-items-center">
                                <div class="timel-year">2020</div>
                                <div>付款方式和Beta版测试，及软启动</div>
                            </div>
                            <div class="d-flex">
                                <div class="timel-bot-col">
                                    <div style="font-weight: 700; font-size: 20px; margin-bottom: 10px">4月 2020</div>
                                    <div class="c-gold">SX APP-BETA V1.0</div>
                                    <div style="font-size: 14px;">侦察模式版本</div>
                                </div>
                                <div class="timel-bot-col">
                                    <div style="font-weight: 700; font-size: 20px; margin-bottom: 10px">5月 2020</div>
                                    <div class="c-gold">SX APP BETA V2.0</div>
                                    <div style="font-size: 14px;">SportXchange</div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div id="investAthleteSct" class="section fp-auto-height">
        <div class="container">
            <div class="my-row">
                <div class="my-col-6 phone-toggle-wrap d-flex flex-column justify-content-center">
                    <div class="phone-toggle-inner d-flex justify-content-between align-items-center">
                        <div class="phones-tab-wrap">
                            <img id="badminton" class="tabcontent" width="300" src="<?php echo get_stylesheet_directory_uri(); ?>/images/phone-1.png" alt="">
                            <img id="soccer" class="tabcontent" width="300" src="<?php echo get_stylesheet_directory_uri(); ?>/images/phone-2.png" alt="">
                            <img id="basketball" class="tabcontent" width="300" src="<?php echo get_stylesheet_directory_uri(); ?>/images/phone-3.png" alt="">
                            <img id="tennis" class="tabcontent" width="300" src="<?php echo get_stylesheet_directory_uri(); ?>/images/phone-4.png" alt="">
                        </div>
                        <div class="tabs-toggles">
                            <div class="tabs-toggles-inner">
                                <div class="py-10">
                                    <a class="tablinks" id="defaultOpen" onclick="switchPhone(event, 'badminton')">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="96" height="96" viewBox="0 0 96 96">
                                            <g id="badminton" transform="translate(-760.5 -3544)">
                                                <g id="Ellipse_56" data-name="Ellipse 56" transform="translate(760.5 3544)" fill="#151515" stroke="red" stroke-width="2">
                                                    <circle cx="48" cy="48" r="48" stroke="none" />
                                                    <circle class="tog-circle" cx="48" cy="48" r="47" fill="none" />
                                                </g>
                                                <g id="Group_1146" data-name="Group 1146" transform="matrix(0.643, 0.766, -0.766, 0.643, 999.913, 3901.52)">
                                                    <path id="Path_1582" data-name="Path 1582" d="M-352.288-44.666c-.066.43-.135.86-.2,1.291-.03.21-.045.422-.072.676h3.716c.217-.419.34-.47.926-.344l-.132.52c.333.056.663.106.991.17a4.1,4.1,0,0,1,.444.142l.7-2.374-1.515,1.059-.069-.031c.062-.284.108-.573.189-.851,1.283-4.372,2.551-8.747,3.862-13.11,1.155-3.842,2.358-7.67,3.54-11.5.02-.065.046-.127.081-.224.066.087.119.151.166.219,1.1,1.585,2.2,3.169,3.292,4.76a1.128,1.128,0,0,1,.148.482c.025.192-.008.391.008.585a9.186,9.186,0,0,1-.641,3.96,48.23,48.23,0,0,1-4,8.773,22.947,22.947,0,0,1-3.781,4.743.745.745,0,0,0-.193.314q-2.114,7.079-4.2,14.165a2.868,2.868,0,0,0,.066,1.025c.082.793.172,1.586.262,2.408h-18.555c.229-.91.447-1.815.687-2.715a1.558,1.558,0,0,0-.023-.934q-2.308-7.064-4.594-14.135a.772.772,0,0,0-.167-.3A30.449,30.449,0,0,1-378.03-56.56a36.671,36.671,0,0,1-1.253-4.091,12.661,12.661,0,0,1-.15-2.062,3.2,3.2,0,0,1,.711-2.265c.985-1.318,1.89-2.7,2.829-4.049.044-.063.093-.123.19-.248l6.884,25.287-.1.061-1.158-.8.986,2.789,1.835-.614-.7-2.194.3-.062-7.66-24.465,1.416.541c1.2.459,2.4.914,3.59,1.382a.337.337,0,0,0,.428-.091q1.782-1.8,3.578-3.583c.072-.072.146-.141.264-.255l2.8,26.486-2.377-2.39c-.313.7-.564,1.32-.865,1.915a1.365,1.365,0,0,0-.076,1.245c.156.363.23.762.357,1.138.026.077.13.181.2.182.966.009,1.931,0,2.9,0,.017,0,.034-.016.071-.033l-.178-1.539h1.2l.149,1.561h3.462v-1.964h.81v1.957h3.9c.046-.462.1-.924.134-1.387a.238.238,0,0,1,.207-.259c.354-.088.7-.2,1.054-.3ZM-361.6-31.009l-.631-5.2L-364.14-36l1.462,4.881c-.792.148-.841.12-1.074-.595-.012-.037-.023-.074-.035-.111L-365.1-35.87a4.933,4.933,0,0,0-1.7.426l1.568,4.435Zm7.408-6.052.4-4.406h-3.763v4.406Zm-.55,6.041.434-4.8h-3.249v4.8Zm4.125-5.659,1.434-4.782h-3.52l-.529,4.482Zm-7.78-4.785h-3.348l.4,4.4h2.944Zm.007,10.447a.79.79,0,0,0,.021-.114c0-1.511,0-3.022.007-4.533,0-.2-.092-.24-.262-.256-.756-.07-1.511-.15-2.266-.223-.119-.011-.241,0-.369,0l.468,5.127Zm-3.941-6.028-.536-4.417h-2.908c.43,1.426.85,2.828,1.279,4.228.023.074.112.18.173.181C-363.676-37.039-363.025-37.044-362.333-37.044Zm8.956,1.266L-353.939-31c.228,0,.44,0,.652,0,.319-.006.718.094.935-.059.2-.143.213-.559.3-.856.358-1.187.714-2.375,1.079-3.591ZM-365.5-37.1l-1.415-4.373-1.883.378,1.643,4.641Zm18.666-4.37h-1.534l-1.428,4.919,1.435.284Zm-1.747,5.924a1.662,1.662,0,0,1-.348.091,8.535,8.535,0,0,1-.934,0c-.213-.015-.285.077-.336.261-.253.905-.516,1.808-.777,2.712-.139.482-.281.962-.43,1.476.48,0,.918.008,1.354-.009.069,0,.168-.111.193-.19.16-.5.3-1,.451-1.5C-349.125-33.662-348.847-34.617-348.577-35.541Z" transform="translate(0 -6.382)" fill="none" stroke="#d0ad6a" stroke-width="1.5" />
                                                    <path id="Path_1583" data-name="Path 1583" d="M-171.316,705.067h.417l18.141.006c.407,0,.4,0,.411.413a13.629,13.629,0,0,1-.876,5.525,8.553,8.553,0,0,1-4.135,4.725,7.778,7.778,0,0,1-2.031.606,15.319,15.319,0,0,1-5.116-.051,5.745,5.745,0,0,1-2.762-1.381,10.635,10.635,0,0,1-3.892-6.076,10.01,10.01,0,0,1-.152-1.768C-171.335,706.414-171.316,705.761-171.316,705.067Z" transform="translate(-195.923 -737.228)" fill="none" stroke="#d0ad6a" stroke-width="1.5" />
                                                    <path id="Path_1584" data-name="Path 1584" d="M83.92-38.029c.038-.484.067-.969.117-1.452.145-1.395.3-2.789.448-4.183.146-1.346.3-2.691.441-4.037.128-1.181.251-2.363.378-3.544q.187-1.743.377-3.486t.382-3.515q.2-1.8.39-3.6.121-1.118.245-2.236c0-.036.015-.072.03-.144.561.457,1.1.9,1.645,1.341.868.71,1.74,1.415,2.6,2.135a.336.336,0,0,0,.422.019l3.848-1.75c.088-.04.177-.077.331-.144L87.516-36.865,86.3-39.673c-.727.485-1.43.951-2.13,1.422a2.555,2.555,0,0,0-.252.221Z" transform="translate(-436.208 -13.018)" fill="none" stroke="#d0ad6a" stroke-width="1.5" />
                                                    <path id="Path_1585" data-name="Path 1585" d="M-8.657-50.785l-1.575-3.132-2.41,2.42c-.135-8.878-.269-17.7-.4-26.56.116.057.209.1.3.147,1.3.706,2.6,1.41,3.894,2.125a.5.5,0,0,0,.511.015c1-.457,2.005-.9,3.009-1.343.106-.047.213-.09.352-.148L-8.6-50.79Z" transform="translate(-344.925)" fill="none" stroke="#d0ad6a" stroke-width="1.5" />
                                                    <path id="Path_1586" data-name="Path 1586" d="M-143.8-77.309l.9.445c.975.486,1.951.969,2.922,1.463a.334.334,0,0,0,.415-.036c.922-.7,1.853-1.4,2.781-2.09.06-.045.123-.084.217-.148v26.863l-1.938-1.98-1.664,2.062Z" transform="translate(-221.833 -0.36)" fill="none" stroke="#d0ad6a" stroke-width="1.5" />
                                                </g>
                                            </g>
                                        </svg>

                                    </a>
                                </div>
                                <div class="py-10">
                                    <a class="tablinks" onclick="switchPhone(event, 'soccer')">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="96" height="96" viewBox="0 0 96 96">
                                            <g id="soccer" transform="translate(-760.5 -3664)">
                                                <g id="Ellipse_57" data-name="Ellipse 57" transform="translate(760.5 3664)" fill="#151515" stroke="red" stroke-width="2">
                                                    <circle cx="48" cy="48" r="48" stroke="none" />
                                                    <circle class="tog-circle" cx="48" cy="48" r="47" fill="none" />
                                                </g>
                                                <g id="Group_683" data-name="Group 683" transform="translate(785.5 3690.078)">
                                                    <g id="Group_512" data-name="Group 512" transform="translate(0 0)">
                                                        <g id="Group_511" data-name="Group 511">
                                                            <path id="Path_568" data-name="Path 568" d="M42.076,9.38A23.512,23.512,0,0,0,9.518,4.454,22.757,22.757,0,0,0,.277,19.479a22.514,22.514,0,0,0,2.9,15.056l.005.01A23.214,23.214,0,0,0,17.964,45.312c.246.057.492.109.739.158a.937.937,0,0,0,.157.03,23.576,23.576,0,0,0,8.891,0,.875.875,0,0,0,.128-.024,23.363,23.363,0,0,0,5.889-2,.891.891,0,0,0,.4-1.2.916.916,0,0,0-1.222-.4A21.888,21.888,0,0,1,29.5,43.24l3.773-6.447,7.386-1.368a20.873,20.873,0,0,1-2.149,2.485.889.889,0,0,0,0,1.268.92.92,0,0,0,1.287,0,22.812,22.812,0,0,0,3.577-4.563c.007-.012.015-.023.021-.036a22.8,22.8,0,0,0,3.033-8.893.53.53,0,0,0,.007-.066c.033-.274.061-.547.084-.823A22.7,22.7,0,0,0,42.076,9.38ZM26.557,2.043A21.413,21.413,0,0,1,38.763,8.275l-1.11,9.408L30.52,19.262l-6.289-4.5V8.022l1.4-.99a.888.888,0,0,0,.209-1.251.918.918,0,0,0-1.269-.206L23.26,6.5l-6.268-3.78A21.692,21.692,0,0,1,26.557,2.043ZM17.35,20.569l2.964-2.122,2.964-2.122L29.2,20.569,26.94,27.437H19.614ZM10.595,5.9a21.738,21.738,0,0,1,4.221-2.388l7.55,4.553v6.694l-6.289,4.5L8.916,17.677,7.455,8.669A21.644,21.644,0,0,1,10.595,5.9ZM2.075,19.752a20.914,20.914,0,0,1,3.838-9.226l1.231,7.585L1.826,23.4A21.191,21.191,0,0,1,2.075,19.752Zm-.067,6.016,6.4-6.364,7.168,1.586L17.953,28.2l-4.476,6.789L4.561,33.305A20.752,20.752,0,0,1,2.008,25.767Zm3.98,9.717-.07-.1,7.409,1.4,3.784,6.466A21.45,21.45,0,0,1,5.988,35.485Zm21.068,8.336a21.628,21.628,0,0,1-7.508.008l-4.573-7.815,4.472-6.784h7.7l4.472,6.784ZM42,33.35l-8.878,1.644L28.644,28.2l2.378-7.214L38.184,19.4l6.349,6.427A21.083,21.083,0,0,1,42,33.35Zm2.723-9.9L39.434,18.1l.94-7.966c.079.1.157.2.235.308A20.9,20.9,0,0,1,44.723,23.452Z" transform="translate(-0.002 0.001)" fill="#d0ad6a" />
                                                        </g>
                                                    </g>
                                                    <g id="Group_514" data-name="Group 514" transform="translate(26.732 3.663)">
                                                        <g id="Group_513" data-name="Group 513">
                                                            <path id="Path_569" data-name="Path 569" d="M295.333,40.527a.909.909,0,1,0,.267.643A.917.917,0,0,0,295.333,40.527Z" transform="translate(-293.78 -40.26)" fill="#d0ad6a" />
                                                        </g>
                                                    </g>
                                                    <g id="Group_516" data-name="Group 516" transform="translate(35.521 39.8)">
                                                        <g id="Group_515" data-name="Group 515">
                                                            <path id="Path_570" data-name="Path 570" d="M391.923,444.977a.91.91,0,1,0,.267.643A.917.917,0,0,0,391.923,444.977Z" transform="translate(-390.37 -444.71)" fill="#d0ad6a" />
                                                        </g>
                                                    </g>
                                                </g>
                                            </g>
                                        </svg>

                                    </a>
                                </div>
                                <div class="py-10">
                                    <a class="tablinks" onclick="switchPhone(event, 'basketball')">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="96" height="96" viewBox="0 0 96 96">
                                            <g id="basketball" transform="translate(-760.5 -3784)">
                                                <g id="Ellipse_58" data-name="Ellipse 58" transform="translate(760.5 3784)" fill="#151515" stroke="red" stroke-width="2">
                                                    <circle cx="48" cy="48" r="48" stroke="none" />
                                                    <circle class="tog-circle" cx="48" cy="48" r="47" fill="none" />
                                                </g>
                                                <g id="Group_1148" data-name="Group 1148" transform="translate(1461.217 2643.179)">
                                                    <path id="Path_1590" data-name="Path 1590" d="M-375.119,1183.37h-6.964v-7.817h-1.974c-.348,0-.7.02-1.045,0a1.117,1.117,0,0,0-1.209.574,2.316,2.316,0,0,1-2.231,1.043.9.9,0,0,0-.951.526,59.6,59.6,0,0,0-6.688,18.21,71.408,71.408,0,0,0-1.438,16.279c0,.055,0,.11,0,.165a.645.645,0,0,1-.447.781c-.393.113-.595-.146-.789-.44-.607-.916-1.227-1.823-1.846-2.732-.1-.148-.22-.285-.4-.519-.846,1.026-1.672,2-2.463,3-.6.753-.868.78-1.485.059l-2.34-2.735c-.574.721-1.128,1.416-1.68,2.112-.217.273-.42.556-.647.821-.361.421-.666.445-1.062.047-.994-1-1.971-2.02-3.012-3.091-.4.574-.8,1.148-1.2,1.726a11.855,11.855,0,0,1-.841,1.219,1.115,1.115,0,0,1-.76.359c-.377-.013-.42-.372-.427-.708-.058-2.565-.01-5.14-.21-7.694-.759-9.662-3.243-18.769-8.894-26.82a1.139,1.139,0,0,0-1.066-.566,2.554,2.554,0,0,1-2.617-2.6,2.562,2.562,0,0,1,2.666-2.549q19.34,0,38.681-.006a2.67,2.67,0,0,1,2.228,1.086.684.684,0,0,0,.413.283c1.208.022,2.417.013,3.689.013v-8.867h7.011Zm-5.03-16.911V1181.4h3.086v-14.937Zm-27.63,8.993h19.036c.147,0,.294.006.44,0a.8.8,0,0,0,.814-.741.8.8,0,0,0-.572-.945,2.45,2.45,0,0,0-.756-.082q-18.981-.006-37.962,0a2.3,2.3,0,0,0-.8.112.781.781,0,0,0-.508.931.805.805,0,0,0,.781.721c.164.013.33.006.495.006Zm.065,15.158c.881-1.036,1.741-2.035,2.58-3.052a.6.6,0,0,0,.064-.473c-.292-1-.678-1.971-.9-2.982-.479-2.16-.875-4.338-1.318-6.506a1.484,1.484,0,0,0-.251-.416c-.1.144-.264.275-.3.435-.168.674-.3,1.357-.445,2.036-.564,2.6-.776,5.286-2.056,7.709Zm7.051-.146c.7-.918,1.331-1.786,2-2.625a1.09,1.09,0,0,0,.228-1.082,11.3,11.3,0,0,1-.282-1.95c-.2-2.371-.362-4.745-.562-7.115a.686.686,0,0,0-.334-.5c-.356-.14-.453.162-.528.439q-.43,1.587-.867,3.172c-.606,2.2-1.005,4.462-2.2,6.461a.429.429,0,0,0,.084.375C-402.326,1188.572-401.515,1189.49-400.664,1190.463Zm-14.218.133c.885-1.064,1.7-2.028,2.49-3.014a.62.62,0,0,0,.036-.513c-.316-.835-.734-1.638-.985-2.492-.628-2.139-1.183-4.3-1.776-6.448-.089-.323-.225-.633-.395-1.1a3.527,3.527,0,0,0-.165.506q-.333,2.916-.656,5.834a8.986,8.986,0,0,1-.852,3.712.439.439,0,0,0,.039.385C-416.416,1188.494-415.671,1189.51-414.882,1190.6Zm-1.961-13.387h-6.594c2.241,2.763,3.493,6.014,5.339,8.948C-417.252,1183.233-417.576,1180.174-416.842,1177.209Zm19.46,8.908.241.06,4.912-8.975h-5.785Zm-13.941.38a43.808,43.808,0,0,0,2.212-9.29h-5.034C-413.028,1180.25-412.693,1183.478-411.323,1186.5Zm5.2-9.3c.746,3.047,1.006,6.125,2.063,9.046,1.358-2.886,1.848-6.012,2.78-9.046Zm5.639,22.351,3.073-3.862-3.118-3.38-2.843,3.635Zm-10.777-11.382c-.908,1.088-1.789,2.134-2.651,3.2a.438.438,0,0,0-.009.416c.892,1.13,1.8,2.245,2.75,3.413l2.694-3.581Zm-3.653,4.363c-.729.882-1.395,1.73-2.11,2.534a.729.729,0,0,0-.013,1.064c.5.653,1.011,1.3,1.521,1.946.378.479.763.954,1.174,1.466l2.429-3.333Zm13.629,7.977-2.865-3.577-2.659,3.4,2.842,3.388Zm-6.335-1.128,2.679-3.425-2.741-3.369-2.674,3.566Zm.7-7.789,2.755,3.378,2.75-3.525-2.786-3.123Zm-.5,16.258,2.641-3.183-2.809-3.353-2.546,3.328Zm-3.685-10.682c-.787,1.08-1.54,2.092-2.258,3.127a.608.608,0,0,0,.079.545c.477.625.989,1.223,1.493,1.827.275.329.559.651.86,1l2.533-3.3Zm7.918,7.519,2.24,2.855c.743-.785,1.5-1.486,2.117-2.29a1.361,1.361,0,0,0,.126-1.223,29.7,29.7,0,0,0-1.812-2.544Zm-10.623,2.869,2.11-2.869-2.6-3.129a10.964,10.964,0,0,0-1.583,2.2,2.468,2.468,0,0,0,.164,1.954A10.411,10.411,0,0,0-413.806,1207.554Zm.753,1.045,2.719,2.9,2.129-2.693-2.681-3.159Zm9.061-2.966-2.616,3.157,2.24,2.612,2.56-2.98Zm6.295-17.061-2.086,2.758,2.687,2.868c.342-1.3.678-2.542.988-3.793.025-.1-.1-.261-.188-.359C-396.741,1189.564-397.2,1189.093-397.7,1188.572Zm-17.99,2.973-2.284-3.087c-.579.8-1.009,1.559-.485,2.528a4.958,4.958,0,0,1,.311,1.049c.162.588.32,1.177.5,1.856Zm24.48-13.307-.138-.071c-.508.893-1.022,1.783-1.522,2.68a44.505,44.505,0,0,1-3.782,6.535.366.366,0,0,0-.052.33c.3.414.627.807,1.015,1.294A58.177,58.177,0,0,1-391.207,1178.237Zm-28.146,9.994c.457-.549.6-1,.086-1.532a4.207,4.207,0,0,1-.586-.987c-.826-1.548-1.619-3.115-2.48-4.644-.576-1.022-1.248-1.991-1.876-2.984l-.142.084A46.673,46.673,0,0,1-419.353,1188.231Zm21.484,10-1.855,2.3,1.3,1.68Zm-18.337,3.83,1.111-1.5-1.479-1.844-.088.034Zm16,6.476,1.345,1.88v-3.294Zm-14.407.156-1.141-1.217v2.838Z" transform="translate(-247.876)" fill="#d0ad6a" />
                                                </g>
                                            </g>
                                        </svg>

                                    </a>
                                </div>
                                <div class="py-10">
                                    <a class="tablinks" onclick="switchPhone(event, 'tennis')">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="96" height="96" viewBox="0 0 96 96">
                                            <g id="tenis" transform="translate(-760.5 -3904)">
                                                <g id="Ellipse_59" data-name="Ellipse 59" transform="translate(760.5 3904)" fill="#151515" stroke="red" stroke-width="2">
                                                    <circle cx="48" cy="48" r="48" stroke="none" />
                                                    <circle class="tog-circle" cx="48" cy="48" r="47" fill="none" />
                                                </g>
                                                <g id="Group_1147" data-name="Group 1147" transform="translate(392.284 3924.408)">
                                                    <path id="Path_1587" data-name="Path 1587" d="M422.362,0a16.506,16.506,0,0,1,8.011,3.408,22.487,22.487,0,0,1,8.274,14.964c.724,5.57-1.1,10.308-4.79,14.529,2.537,2.269,5.036,4.546,7.582,6.77,2.727,2.383,2.846,4.872.288,7.45-.512.517-1.012,1.047-1.549,1.538a4.252,4.252,0,0,1-6.249-.265c-2.061-2.222-4.053-4.507-6.077-6.763-.422-.471-.855-.933-1.5-1.641.181,3.359.281,6.4.528,9.434.243,2.991-1.541,5.113-4.532,5.181-.808.018-1.617.017-2.426,0a4.356,4.356,0,0,1-4.574-4.794c.092-2.219.291-4.433.422-6.651.038-.635.006-1.275.006-1.592a52.23,52.23,0,0,1-7.13-2.61c-5.652-3.04-8.518-8.174-9.815-14.266-1.018-4.786-.577-9.4,2.155-13.6a13.967,13.967,0,0,1,7.986-5.731,4.181,4.181,0,0,0,1.56-.829A15.774,15.774,0,0,1,419.378.17a1,1,0,0,0,.315-.17ZM401.21,20.6a18.269,18.269,0,0,0,1.751,7.945c2.1,4.632,5.352,8.024,10.432,9.318a1.613,1.613,0,0,0,1.3-.349c5.429-5.374,10.808-10.8,16.242-16.168a1.793,1.793,0,0,0,.494-2.118,14.94,14.94,0,0,0-6.3-8.315,19.57,19.57,0,0,0-14.177-3.132C404.98,8.758,401.219,13.614,401.21,20.6Zm16.9,17.533.134.438c1.6-.455,3.239-.806,4.791-1.389,2.518-.946,4.039-.639,5.819,1.37q3.415,3.852,6.866,7.672c1.083,1.194,1.708,1.189,2.828.089.49-.481.977-.965,1.455-1.458,1.065-1.1,1.024-1.78-.149-2.823-2.473-2.2-4.952-4.4-7.414-6.61-2.325-2.092-2.558-3.377-1.478-6.315.542-1.475.833-3.042,1.305-4.819a3.784,3.784,0,0,0-.636.315Q424.87,31.368,418.113,38.138Zm16.6-12.164.405.007a32.633,32.633,0,0,0,.805-4.1,18.691,18.691,0,0,0-4.748-13.8c-2.9-3.41-6.461-5.581-11.166-5.165A10.369,10.369,0,0,0,415.02,4.7c5.829.528,10.786,2.42,14.749,6.426C433.827,15.23,435.295,20.269,434.716,25.974ZM423.549,40.211c-1.467.293-2.848.526-4.2.874a1.206,1.206,0,0,0-.709.861c-.19,2.773-.328,5.551-.434,8.329a1.312,1.312,0,0,0,1.412,1.463c.886.034,1.774.027,2.661.011,1.334-.023,1.835-.5,1.777-1.839C423.917,46.693,423.724,43.477,423.549,40.211Z" transform="translate(-2.353)" fill="#d0ad6a" />
                                                    <path id="Path_1588" data-name="Path 1588" d="M395.216,169.688a5.44,5.44,0,0,1,5.578-5.576,5.579,5.579,0,0,1,5.614,5.6,5.757,5.757,0,0,1-5.583,5.593A5.6,5.6,0,0,1,395.216,169.688Zm8.321,0a2.779,2.779,0,0,0-2.8-2.849,2.966,2.966,0,0,0-2.794,2.8,2.775,2.775,0,0,0,2.857,2.785A2.676,2.676,0,0,0,403.537,169.691Z" transform="translate(0 -124.101)" fill="#d0ad6a" />
                                                </g>
                                            </g>
                                        </svg>

                                    </a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="my-col-6 phone-tab-cnt-right">
                    <img style="margin-bottom: 20px;" src="<?php echo get_stylesheet_directory_uri() ?>/images/txchange-logo.svg" alt="">
                    <h2 style="line-height: 1; margin-bottom: 20px"><span class="c-gold">INVEST*</span> <span class="light-col">IN YOUR<br>
                            ATHLETE’S DREAMS</span></h2>
                    <p class="light-col" style="margin-bottom: 20px;">The SportXchange™ is the first people’s sub-exchange that will be launched under TheXchange™ in the second quarter of 2020.</p>

                    <p class="light-col" style="margin-bottom: 20px;">There are millions of athletes around the world, driven by unstoppable passion, to be the best in their arena. The SportXchange™ will enable fans, friends and family to “invest” in Professional and Amateur athletes and share in their success (read more)</p>

                    <p class="light-col" style="margin-bottom: 20px;">Driven by TheXchange’s proprietary Global Popularity Index (GPX) algorithm, every athlete on earth could be ranked on the SportXchange™ in the GPX according to their performance.</p>
                    <div class="video-wrap">
                        <img src="<?php echo get_stylesheet_directory_uri() ?>/images/messi-video-holder.jpg" alt="Play Video" />
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div id="thePlatform" class="section fp-auto-height">
        <div class="container">
            <div class="my-row">
                <div class="my-col-6 thexchange-logo">
                    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="264.786" height="288" viewBox="0 0 264.786 288">
                        <defs>
                            <linearGradient id="linear-gradient" x1="-0.141" y1="0.81" x2="2.768" y2="-0.598" gradientUnits="objectBoundingBox">
                                <stop offset="0" stop-color="#fdedc9" />
                                <stop offset="0.042" stop-color="#fae9c4" />
                                <stop offset="0.091" stop-color="#f4e0b6" />
                                <stop offset="0.142" stop-color="#e9d09f" />
                                <stop offset="0.196" stop-color="#dabb7f" />
                                <stop offset="0.225" stop-color="#d0ad6a" />
                                <stop offset="0.243" stop-color="#d6b677" />
                                <stop offset="0.288" stop-color="#e4c994" />
                                <stop offset="0.337" stop-color="#efd9ab" />
                                <stop offset="0.389" stop-color="#f6e4bc" />
                                <stop offset="0.447" stop-color="#fbeac5" />
                                <stop offset="0.523" stop-color="#fdedc9" />
                                <stop offset="0.562" stop-color="#fae9c4" />
                                <stop offset="0.608" stop-color="#f4e0b6" />
                                <stop offset="0.656" stop-color="#e9d09f" />
                                <stop offset="0.707" stop-color="#dabb7f" />
                                <stop offset="0.734" stop-color="#d0ad6a" />
                                <stop offset="0.89" stop-color="#b18a4b" />
                                <stop offset="1" stop-color="#9a7034" />
                            </linearGradient>
                            <linearGradient id="linear-gradient-2" x1="-0.35" y1="0.914" x2="2.541" y2="-0.494" xlink:href="#linear-gradient" />
                            <linearGradient id="linear-gradient-3" x1="-0.861" y1="1.529" x2="4.05" y2="-1.684" xlink:href="#linear-gradient" />
                            <linearGradient id="linear-gradient-4" x1="-0.715" y1="1.158" x2="4.3" y2="-2.133" xlink:href="#linear-gradient" />
                        </defs>
                        <g id="Group_974" data-name="Group 974" transform="translate(-0.876 -13)">
                            <g id="Group_894" data-name="Group 894" transform="translate(0.876 13)">
                                <g id="logotext" transform="translate(0 252.698)">
                                    <path id="Path_1439" data-name="Path 1439" d="M11.527,972.229h-.769q-.44,0-.439-.494V938.9q0-.438-.494-.439H1.37c-.33,0-.494-.146-.494-.439v-.659c0-.292.165-.439.494-.439H20.915c.329,0,.494.147.494.439v.659c0,.294-.165.439-.494.439H12.46q-.494,0-.494.439v32.832Q11.966,972.229,11.527,972.229Z" transform="translate(-0.876 -936.927)" />
                                    <path id="Path_1440" data-name="Path 1440" d="M91.209,972.229H90.44c-.293,0-.439-.165-.439-.494V937.421c0-.329.146-.494.439-.494h.769q.439,0,.439.494v15.592c0,.293.165.439.494.439h17.019c.329,0,.494-.146.494-.439V937.421c0-.329.146-.494.439-.494h.769q.439,0,.439.494v34.314q0,.494-.439.494H110.1c-.293,0-.439-.165-.439-.494V955.429q0-.439-.494-.439H92.142q-.495,0-.494.439v16.306Q91.648,972.229,91.209,972.229Z" transform="translate(-65.625 -936.927)" />
                                    <path id="Path_1441" data-name="Path 1441" d="M213.115,972.229h-16.69c-.293,0-.439-.165-.439-.494V937.421c0-.329.146-.494.439-.494h16.69c.33,0,.494.147.494.439v.659c0,.294-.165.439-.494.439H198.127q-.494,0-.494.439v14.11c0,.293.165.439.494.439h13.561c.329,0,.494.147.494.439v.659c0,.294-.165.439-.494.439H198.127q-.494,0-.494.439v14.823c0,.294.165.439.494.439h14.988q.494,0,.494.439v.659C213.609,972.083,213.445,972.229,213.115,972.229Z" transform="translate(-142.623 -936.927)" />
                                    <path id="Path_1442" data-name="Path 1442" d="M276.618,972.229h-4.063a.714.714,0,0,1-.631-.275.671.671,0,0,1,.028-.714l9.059-17.294-8.674-16.031a.624.624,0,0,1-.027-.686.69.69,0,0,1,.631-.3h3.9a1.562,1.562,0,0,1,1.537.934l6.314,12.737h.219l6.314-12.737a1.55,1.55,0,0,1,1.428-.934h4.063a.674.674,0,0,1,.631.329.756.756,0,0,1,.028.713l-8.784,15.977,9.059,17.239a.78.78,0,0,1,.055.741.681.681,0,0,1-.659.3h-4.118a1.3,1.3,0,0,1-1.263-.879l-6.753-13.4h-.219l-6.753,13.4A1.34,1.34,0,0,1,276.618,972.229Z" transform="translate(-197.726 -936.927)" />
                                    <path id="Path_1443" data-name="Path 1443" d="M391.52,972.229h-6.369a7.772,7.772,0,0,1-8.51-8.51V945.437a7.738,7.738,0,0,1,8.51-8.51h6.369a7.772,7.772,0,0,1,8.51,8.51v2.635a.874.874,0,0,1-.988.988h-3.459q-.934,0-.933-.988v-2.416q0-4.062-3.953-4.063H385.92q-3.953,0-3.953,4.063V963.5q0,4.064,3.953,4.063H390.7q3.953,0,3.953-4.063v-2.415q0-.989.933-.989h3.459a.874.874,0,0,1,.988.989v2.635a7.806,7.806,0,0,1-8.51,8.51Z" transform="translate(-273.868 -936.927)" />
                                    <path id="Path_1444" data-name="Path 1444" d="M483.407,972.229h-3.459a.824.824,0,0,1-.933-.933V937.861a.825.825,0,0,1,.933-.934h3.459a.825.825,0,0,1,.933.934v13.725q0,.549.659.549h11.584a.485.485,0,0,0,.549-.549V937.861a.825.825,0,0,1,.933-.934h3.514a.825.825,0,0,1,.933.934V971.3a.825.825,0,0,1-.933.933h-3.514a.824.824,0,0,1-.933-.933v-13.89a.486.486,0,0,0-.549-.549H485q-.658,0-.659.549V971.3A.825.825,0,0,1,483.407,972.229Z" transform="translate(-348.242 -936.927)" />
                                    <path id="Path_1445" data-name="Path 1445" d="M581.142,972.229h-3.953q-.769,0-.549-.933l9.553-33.435a1.232,1.232,0,0,1,1.318-.934H593a1.184,1.184,0,0,1,1.318.934l9.553,33.435q.219.934-.6.933h-3.9a.959.959,0,0,1-.687-.192,1.491,1.491,0,0,1-.3-.741l-2.086-7.137H584.216L582.13,971.3Q581.91,972.23,581.142,972.229Zm9-30.032-4.831,17.569h9.937L590.365,942.2Z" transform="translate(-419.13 -936.927)" />
                                    <path id="Path_1446" data-name="Path 1446" d="M692.512,972.229h-3.4a.824.824,0,0,1-.933-.933V937.861a.825.825,0,0,1,.933-.934h3.075a1.253,1.253,0,0,1,1.208.6l13.286,23.882h.219V937.861a.825.825,0,0,1,.933-.934h3.4a.826.826,0,0,1,.934.934V971.3a.825.825,0,0,1-.934.933h-2.965a1.634,1.634,0,0,1-1.427-.879l-13.176-23.608h-.22V971.3A.825.825,0,0,1,692.512,972.229Z" transform="translate(-500.196 -936.927)" />
                                    <path id="Path_1447" data-name="Path 1447" d="M809.549,972.229h-5.875a7.772,7.772,0,0,1-8.51-8.51V945.437a7.738,7.738,0,0,1,8.51-8.51h5.875a7.738,7.738,0,0,1,8.51,8.51v1.977q0,.988-.933.988h-3.459q-.934,0-.933-.988v-1.757q0-4.062-3.953-4.063h-4.337q-3.953,0-3.953,4.063V963.5q0,4.064,3.953,4.063h4.337q3.953,0,3.953-4.063v-4.941c0-.365-.2-.549-.6-.549h-4.447a.824.824,0,0,1-.933-.933v-2.8a.825.825,0,0,1,.933-.933h9.223a1.019,1.019,0,0,1,1.153,1.153v9.223a7.806,7.806,0,0,1-8.51,8.51Z" transform="translate(-577.924 -936.927)" />
                                    <path id="Path_1448" data-name="Path 1448" d="M916.643,972.229H899.075a.824.824,0,0,1-.933-.933V937.861a.825.825,0,0,1,.933-.934h17.568q.878,0,.878.934v2.8q0,.934-.878.933H904.126q-.658,0-.659.549v9.278q0,.549.659.549h10.6q.988,0,.988.933v2.8q0,.934-.988.933h-10.6q-.658,0-.659.549v9.773q0,.6.659.6h12.518q.878,0,.878.933v2.8Q917.522,972.23,916.643,972.229Z" transform="translate(-652.736 -936.927)" />
                                </g>
                                <path id="Path_1449" data-name="Path 1449" d="M116.268,233.375H75.69L208.792,13H249.37Z" transform="translate(-55.228 -13)" fill="#d3b37f" />
                                <path id="Path_1450" data-name="Path 1450" d="M295.112,233.375H253.441L386.543,13h41.672Z" transform="translate(-184.363 -13)" fill="#d3b37f" />
                                <path id="Path_1451" data-name="Path 1451" d="M615.623,550.475l-61-101-44.751,72.879,16.983,28.118Z" transform="translate(-370.66 -330.099)" fill="#d3b37f" />
                                <path id="Path_1452" data-name="Path 1452" d="M187.021,38.883,171.388,13H83.338L143,111.775Z" transform="translate(-60.784 -13)" fill="#d3b37f" />
                                <g id="Group_893" data-name="Group 893" transform="translate(22.921 1.388)">
                                    <path id="Path_1453" data-name="Path 1453" d="M84.68,235.674l131.426-217.6h37.336l-131.426,217.6Z" transform="translate(-84.68 -18.073)" fill="url(#linear-gradient)" />
                                    <path id="Path_1454" data-name="Path 1454" d="M262.434,235.674l131.425-217.6h38.43l-131.426,217.6Z" transform="translate(-213.817 -18.073)" fill="url(#linear-gradient-2)" />
                                    <path id="Path_1455" data-name="Path 1455" d="M531.953,556.173l-16.141-26.725,43.116-70.217,58.551,96.942Z" transform="translate(-397.896 -338.573)" fill="url(#linear-gradient-3)" />
                                    <path id="Path_1456" data-name="Path 1456" d="M92.327,18.073h84.809l14.794,24.5-42.4,70.208Z" transform="translate(-90.235 -18.073)" fill="url(#linear-gradient-4)" />
                                </g>
                            </g>
                            <g id="logotm" transform="translate(234.61 202.946)">
                                <path id="Path_1457" data-name="Path 1457" d="M859.884,707.492v.99h-1.646v4.631H857.1v-4.631h-1.633v-.99Zm2.075,0,1.713,3.573,1.713-3.573H866.7v5.621h-1.031v-3.975l-1.619,3.4h-.75l-1.606-3.4v3.975h-1.044v-5.621Z" transform="translate(-855.467 -707.492)" />
                            </g>
                        </g>
                    </svg>
                </div>
                <div class="my-col-6">
                    <h2 class="c-gold">The platform</h2>
                    <p>Our platform will have the ability to generate specialized sub-exchanges worldwide, in any language and in any currency. It will follow the same unique and insanely-simple business model that can be duplicated regionally internationally, nationally, and even for event-specific exchanges.</p>
                    <button class="my-btn"><span>Buy turncoin</span></button>
                </div>
            </div>
        </div>
        <div class="turning-coin-wrapper">
            <div id="turningCoin">
                <img src="<?php echo get_stylesheet_directory_uri() ?>/images/xchange_coin_large.png" alt id="largecoin">
            </div>
        </div>
    </div>
    <div id="txChangeSection" class="section fp-auto-height">
        <div class="container">
            <div style="margin-bottom: 30px" class="text-center thexchange-logo"><img width="200px" src="<?php echo get_stylesheet_directory_uri() ?>/images/xchange-logo-white.svg" alt=""></div>
            <div class="d-flex">
                <div class="my-col-6"></div>
                <div class="my-col-6 blocky-top-line"></div>
            </div>
            <div style="margin-bottom: 20px;" class="d-flex blocky-wrap">
                <div class="my-col-1-0 blocky"></div>
                <div class="my-col-1-0 blocky"></div>
                <div class="my-col-1-0 blocky"></div>
                <div class="my-col-1-0 blocky"></div>
                <div class="my-col-1-0 blocky"></div>
                <div class="my-col-1-0 blocky b-hide-top"></div>
                <div class="my-col-1-0 blocky b-hide"></div>
                <div class="my-col-1-0 blocky b-hide"></div>
                <div class="my-col-1-0 blocky b-hide"></div>
                <div class="my-col-1-0 blocky b-hide"></div>
            </div>
            <div class="d-flex" style="margin-bottom: 30px;">
                <div class="my-col-20 blocky-5"><img src="<?php echo get_stylesheet_directory_uri() ?>/images/phone-1.png" alt=""></div>
                <div class="my-col-20 blocky-5"><img src="<?php echo get_stylesheet_directory_uri() ?>/images/phone-2.png" alt=""></div>
                <div class="my-col-20 blocky-5"><img src="<?php echo get_stylesheet_directory_uri() ?>/images/phone-3.png" alt=""></div>
                <div class="my-col-20 blocky-5 b-hide"><img src="<?php echo get_stylesheet_directory_uri() ?>/images/phone-4.png" alt=""></div>
                <div class="my-col-20 blocky-5 b-hide"><img src="<?php echo get_stylesheet_directory_uri() ?>/images/phone-5.png" alt=""></div>
            </div>
            <p class="light-col text-center" style="width: 768px; max-width: 100%; margin: 0 auto;">Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod tempor invidunt ut labore et dolore magna aliquyam erat, sed diam voluptua. At vero eos et accusam et justo duo dolores et ea rebum. Stet clita kasd gubergren, no sea takimata sanctus est Lorem</p>
        </div>
    </div>
    <div id="equityStructure" class="section fp-auto-height">
        <div class="container text-center">
            <?php
            $horizontallogo = file_get_contents(get_stylesheet_directory_uri().'/images/the_xchange_horizontal_logo.svg');
            echo $horizontallogo;
            ?>
            <h2>Equity structure</h2>
            <h3 style="font-weight: 200; text-transform: uppercase;"><span class="c-gold" style="font-family: rajdhani; font-weight: 700;">Turn</span><span style="font-family: rajdhani; font-weight: 400;">Coin™</span> digital security</h3>
            <div class="graph-desktop">
                <?php
                $graphic = file_get_contents(get_stylesheet_directory_uri().'/images/graph.svg');
                echo $graphic;
                ?>
            </div>
            <div class="graph-mobile">
                <?php
                $graphic = file_get_contents(get_stylesheet_directory_uri().'/images/graphmobile.svg');
                echo $graphic;
                ?>
            </div>
            <h3 class="graph-finite-text" style="text-transform: uppercase; margin-bottom: 20px;"><span class="c-gold" style="font-family: rajdhani; font-weight: 700;">Turn</span><span style="font-family: rajdhani; font-weight: 400;">Coin™</span> 1,000,000,000 - <span class="c-gold">finite</span></h3>
            <h3 class="graph-sec-text" style="text-transform: uppercase; margin-bottom: 50px;"><span style="font-weight: 500;">sec regulated Security</span> <?php $goldarrow = file_get_contents(get_stylesheet_directory_uri().'/images/goldarrow.svg');
                                                                                                                                                                echo $goldarrow; ?> <span>Monthly yield</span></h3>
            <div style="max-width: 500px; margin: 0 auto;">
                <p style="margin-bottom: 20px;">So, how does this work? Glad you asked.</p>
                <p> Our utility token – the DriveCoin™ - will affect every industry, revolutionize business and impact people worldwid</p>
            </div>
        </div>
    </div>
    <div id="holders" class="section fp-auto-height">
        <div class="container">
            <div class="my-row">
                <div class="my-col-4 text-center">
                    <div class="coin-shine">
                        <img src="<?php echo get_stylesheet_directory_uri() ?>/images/xchange_coin_large.png" alt="">
                    </div>
                    <button class="my-btn light-col"><span>Buy turncoin</span></button>
                </div>
                <div class="my-col-8 holder-text-cnt">
                    <h3 class="c-gold" style="margin-bottom: 20px;">Dynamic dual nature Revenue sharing</h3>
                    <h1 style="font-weight: 800; text-transform: uppercase; margin-bottom: 20px;"><span class="c-gold" style="font-family: rajdhani; font-weight: 700;">Turn</span><span class="light-col" style="font-family: rajdhani; font-weight: 400;">Coin™</span> <span class="light-col">HOLDERS</span></h1>
                    <p class="light-col" style="margin-bottom: 20px;">We are an elite group of like-minded visionaries that share a common belief that TheXchange™ will pioneer a completely new global market that is currently non-existent. It will allow fans to support and, in return benefit financially from the success of athletes.</p>
                    <p class="light-col" style="margin-bottom: 20px;">This will completely change the way in which athletes worldwide can monetize their careers.</p>
                    <p class="light-col">TheXchange’s revolutionary underlying income model will drive perpetual monthly returns through the SportXchange™ and other sub-exchanges (i.e. ActorXchange, MucisianXchange, BusinessXchange, etc.) to TurnCoin™ security token-holders. (Read more)</p>
                </div>
                <div class="my-col-12 text-center" style="">
                    <img src="<?php echo get_stylesheet_directory_uri() ?>/images/pfp.png" alt="">
                </div>
            </div>
        </div>
    </div>
</div>

<?php
// include footer
// include_once('footer.php');
get_footer();
